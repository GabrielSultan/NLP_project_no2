...
Supervised models in the notebook (Section~7) need a target column \texttt{thematique}. The project uses a \textbf{deterministic weak-labelling} procedure so that labels are \textbf{reproducible}, \textbf{cheap}, and \textbf{aligned with insurance-facing axes} (claims, pricing, reimbursements, customer service, contract administration, digital channels, documents, bonuses, and a residual \texttt{autre} class).

\textbf{Rationale.} Keyword rules are easy to audit and version in code; combining them with a small Word2Vec neighbourhood expansion captures near-synonyms and domain variants without hand-curating every surface form. The trade-off is explicit: a single theme per review, a fixed priority order when several rules fire, and inevitable noise---yet the signal is strong enough for a fine-tuned encoder to \textbf{generalise beyond literal keyword hits}, which is the main reason to learn a classifier on top of these targets.

\textbf{Construction (notebook).} A dictionary \texttt{THEMATIQUES} maps each theme to \textbf{truncated or stem-like keyword fragments} consistent with processed text (\texttt{avis\_traite}). For each theme, the first two seed keywords that appear in the trained Word2Vec vocabulary are used to pull up to three \texttt{most\_similar} neighbours each; those neighbours are appended to the theme list (if not already present) to widen coverage without exploding the rule set. The function \texttt{detect\_thematique} tokenises the review, walks themes in a \textbf{fixed order}, and assigns the \textbf{first} theme for which any keyword appears as a whole token; if none match, the label is \texttt{autre}.
...
Classifiers below predict the \texttt{thematique} column built in Section~\ref{sec:deterministic-labels} (nine labels including \texttt{autre}).
...
\textbf{Results (notebook Section~7, held-out test set).} On the same stratified split with 4{,}821 test examples, the linear TF-IDF baseline reaches about 90\% accuracy and a macro-averaged F1 of about 0.69, while weighted F1 stays near 0.89---the gap between macro and weighted scores reflects class imbalance: frequent themes such as \texttt{service\_client} are easy to recover, but rarer labels remain harder for a bag-of-words model. The fine-tuned DistilBERT raises accuracy to about 97\% with macro F1 near 0.95 and weighted F1 near 0.97 (see the notebook's Section~7.3 bar comparison). The confusion matrix (Figure~\ref{fig:distilbert-cm}) is strongly diagonal; remaining errors are small in absolute terms and often involve neighbouring semantics (e.g.\ \texttt{remboursements} vs.\ \texttt{service\_client}, or \texttt{gestion\_contrat} vs.\ \texttt{autre}).

\textbf{Why DistilBERT in the Streamlit app.} The application tab shows calibrated theme probabilities for arbitrary pasted reviews, so the stronger contextual model is preferred over TF-IDF + logistic regression despite the latter's simplicity and speed. DistilBERT captures word order and phrasing beyond n-gram counts, which matters for short, idiomatic French insurance language. Relative to full BERT, DistilBERT keeps fewer parameters and faster inference, which is important because the single-review pipeline already loads separate models for summarization and sentiment; a lighter encoder reduces end-to-end latency and memory use while preserving the accuracy gains observed on the test set.
